...
class _GuestDetailsPageState extends State<GuestDetailsPage> {
  ...
  void initState() {
    super.initState();
    // ✅ Re-fetch when page opens, so GetBuilder is already listening
    Future.microtask(() => Get.find<GuestController>().fetchGuests());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<GuestController>(
        init: Get.find<GuestController>(),
        builder: (guestController) {
          // LOADING

          if (guestController.isLoading) {
          ...
          final billing =
              guestController.billingData?.guestBillingByReservationIdResponse;

          if (billing == null || billing.billingGuestList.isEmpty) {
